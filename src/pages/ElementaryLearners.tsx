...
const ElementaryLearners: React.FC = () => {
  const {
    isDarkMode
  } = useTheme();

  // Mock data for completed levels - in a real app this would come from a database or localStorage
  const completedLevels = [1, 2, 3]; // Example: levels 1, 2 and 3 are completed
      ...
      <main className="flex-1 container mx-auto p-4 relative z-10">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-3xl font-bold text-center mb-8 bg-gradient-to-r from-aishiksha-blue to-aishiksha-purple bg-clip-text text-transparent">
            Elementary Learners (7-10 Years)
          </h1>
          
          <div className="rounded-xl overflow-hidden shadow-xl mb-8">
            <img src="/lovable-uploads/579d765c-58f8-454f-aa03-66f6557de94c.png" alt="Elementary learners" className="w-full h-auto max-h-[40vh] object-contain" />
          </div>
          
          <p className="text-center mb-8">Learn coding basics through fun, interactive exercises designed for young minds.</p>
...
export default ElementaryLearners;